...
@Component({
  selector: 'app-core-month-formly-type',
  templateUrl: './month-formly-type.component.html',
  styleUrls: ['./month-formly-type.component.css']
})

export class MonthFormlyTypeComponent extends FieldType implements OnInit, FormlyExtension {
  ...
  onKeyChange(e: any) {
    ...
    if (e.target.value.match(dateReg)) {
      const date = moment(e.target.value, 'MM/YYYY').toDate();

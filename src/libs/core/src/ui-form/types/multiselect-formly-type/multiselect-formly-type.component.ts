...
@Component({
  selector: 'app-core-multiselect-formly-type',
  templateUrl: './multiselect-formly-type.component.html',
  styleUrls: ['./multiselect-formly-type.component.css']
})
export class MultiselectFormlyTypeComponent extends FieldType implements OnDestroy, OnInit, FormlyExtension {
  optionArray: any;
  URL: string;
  selectedItem: any = '';
  apiConfig: any;
  ...
  constructor(
      private http: HttpClient, private validationsLoader: ValidationLoaderFormlyService) {
    super();
  ...
  ngOnInit() {
    ...
    this.formControl.valueChanges.subscribe(val => {
      ...
      if (val && typeof val === "string" && this.to.options && (this.to.options as []).length > 0 && (val.indexOf(this.to.separator) !== -1)) {
        this.selectedItem = this.filterArray(this.to.options, val.split(this.to.separator));
        setTimeout(() => {
  ...
  convertArrayOptions(options) {
    ...
    if (this.selectedItem && typeof this.selectedItem === "string") {
      ...
      if (this.to.options && (this.to.options as []).length > 0 && arr.length > 0) {
  ...
  onOpen(multiSelect: MultiSelect) {
    ...
    fromEvent(filterInput, 'keydown').pipe(
  ...
  getDynamicUrl(params) {
    ...
    this.http.get(this.saveUrl, { params: params }).toPromise().then(response => {
      if (response) {
        this.to.options = [];
        this.convertArrayOptions(response);
        // @ts-ignore
